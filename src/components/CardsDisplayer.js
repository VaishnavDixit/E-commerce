...
const CardsDisplayer = ({ listOf, cart, liked, setCart, setLiked  }) => {
	...
	return (
		<>
			...
			{
				listOf == 'likes' ? '' : <div style={{ border:'', display: 'flex', alignItems:'center' ,justifyContent:'end'}}><Title level={4} style={{margin:'5px', border:''}}>Sum = {sum.toLocaleString('en-IN')}</Title><Button>Checkout</Button></div>
			}
		</>
	)
}
